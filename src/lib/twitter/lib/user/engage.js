...
export const engage = async (user_id, followUser=true, numOfLikes=3) => {
  const results = {}

  if (followUser) {
    try {
      results.follow = await follow(user_id)
    } catch (error) {
      results.follow = {error}
    }
  }

  const tweets = await userTweet.get(user_id)
  if (tweets.error) {
    ...
    for (let i=0; i<sortedTweets.length; i++) {
      const response = tweet.like(sortedTweets[i].id_str)
